...
directory = './scripts/deep/mnist/info/'
network_names = ['cnnsimple', 'wrn164']
method = 'dfs'


def scheduler(wrn=True, saliency=False):
    def sch(epoch):
        if saliency:
            return .001
        elif not wrn:
...
def main():
    ...
    for network_name in network_names:
        ...
        epochs = 80 if 'wrn' in network_name else 40

        fit_kwargs = {
            'epochs': epochs,
            'callbacks': [
                callbacks.LearningRateScheduler(scheduler('wrn' in network_name, saliency=False))
            ],
            'verbose': 2
        }
        ...
        try:
            ...
            pass
        output_filename = directory + network_name + '_' + str(gamma) + '_dfs_results.json'
